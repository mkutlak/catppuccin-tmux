...
main() {
  ...
  local uptime
  uptime="$(get_tmux_option "@catppuccin_uptime" "off")"
  readonly uptime
  ...
  local uptime_icon
  uptime_icon="$(get_tmux_option "@catppuccin_uptime_icon" "☕")"
  readonly putime_icon
  ...
  if [[ "${uptime}" == "on" ]]; then
    right_column2="$right_column2$show_uptime"
  fi
